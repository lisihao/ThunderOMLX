#!/usr/bin/env python3
"""
Verify A+B fix: Disabled async prefetch + MIN_TOKENS_FOR_CHUNKING=2560

Tests that 2048 tokens now work without crashing.
"""
...
def main():
    ...
    print("🔧 A+B 组合修复验证")
    print("  A: 禁用异步预取 (OMLX_ENABLE_ASYNC_PREFETCH=false)")
    print("  B: 提高最小 chunking 阈值 (MIN_TOKENS_FOR_CHUNKING=2560)")
    print("="*60)
    ...
    # Setup environment with A+B fixes
    env = os.environ.copy()
    env["OMLX_ENABLE_CHUNKED_PREFILL"] = "true"
    env["OMLX_CHUNK_SIZE"] = "512"
    env["OMLX_MIN_TOKENS_FOR_CHUNKING"] = "2560"  # B: 提高到 2560
    env["OMLX_ENABLE_ASYNC_PREFETCH"] = "false"   # A: 禁用异步预取
    ...
    print(f"     ASYNC_PREFETCH=false ← 禁用")
    ...
    log_file = Path(__file__).parent / "verify_ab_fix.log"
    ...
    # Test critical lengths (focus on 2048)
    ...
    if all_passed and len(results) == len(test_lengths):
        print("✅ A+B 修复成功！所有测试通过。")
        print("\n关键改进：")
        print("  ✓ 2048 tokens 不再崩溃")
        print("  ✓ 异步预取已禁用（避免 Metal GPU 并发问题）")
        print("  ✓ Chunking 阈值提高到 2560（避开问题区间）")
        print("\n下一步：运行完整基准测试")
    else:
        print("❌ 仍有问题。请查看日志:")
    ...
    # Save results
    report_file = Path(__file__).parent / "AB_FIX_VERIFICATION.json"
    with open(report_file, "w") as f:
        json.dump({
            "fixes_applied": {
                "A": "禁用异步预取",
                "B": "MIN_TOKENS_FOR_CHUNKING=2560"
            },
