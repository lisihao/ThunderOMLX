...
Phase A: Uses OpenAI text-embedding-3-small API for prompt embeddings.
Phase B (future): Will use local BGE embedding for fully offline operation.

Based on: https://github.com/lm-sys/RouteLLM
Paper: "RouteLLM: Learning to Route LLMs with Preference Data"

Architecture:
  1. Embed prompt via OpenAI API (1536-dim)
...
Usage::
    ...
    router = MFRouter(
        checkpoint_path="models/mf-router/model.safetensors",
        openai_api_key="sk-...",
        threshold=0.5,
    )
...
_OPENAI_TIMEOUT_SECONDS = 5
_MAX_INPUT_CHARS = 8192  # OpenAI API input limit
...
class MFRouter:
    ...
    def __init__(
        ...
        self._text_proj: np.ndarray = tensors["text_proj.0.weight"]    # (128, 1536)
        ...
        if self._text_proj.shape != (_HIDDEN_DIM, _EMBEDDING_DIM):
            raise ValueError(
                f"text_proj.0.weight shape {self._text_proj.shape} incompatible, "
                f"expected ({_HIDDEN_DIM}, {_EMBEDDING_DIM})"
            )
        ...
        # LRU embedding cache to avoid redundant OpenAI API calls
        ...
        # Router is only available when we have an API key for embeddings
        self._available: bool = bool(openai_api_key)

        logger.info(
            "[__init__] MFRouter ready | checkpoint=%s | threshold=%.2f | "
            "strong_id=%d | weak_id=%d | P_shape=%s | api_key=%s",
            checkpoint_path,
            self._threshold,
            strong_model_id,
            weak_model_id,
            self._P.shape,
            "set" if openai_api_key else "NOT SET (disabled)",
        )
    ...
    def default_checkpoint_path(cls) -> str:
        ...
        Searches common locations relative to the package and the user's
        home directory.

        Returns:
            Absolute path string to model.safetensors, or "" if not found.
        """
        candidates = [
            ...
            / "mf-router"
            / "model.safetensors",
            Path.home() / "ThunderOMLX" / "models" / "mf-router" / "model.safetensors",
        ]
    ...
    def available(self) -> bool:
        """Whether the MF Router is available (has an OpenAI API key)."""
    ...
    # ------------------------------------------------------------------
    # Embedding retrieval (OpenAI API + LRU cache)
    # ------------------------------------------------------------------

    async def _get_embedding(self, text: str) -> Optional[np.ndarray]:
        """Get text embedding from OpenAI API with LRU caching.

        Uses the first ``_CACHE_KEY_PREFIX_LEN`` characters as the cache key,
        since the most routing-relevant information is typically at the
        ...
        Returns:
            1-D numpy array of shape ``(1536,)`` with dtype float32,
        ...
        # Truncate input to API limit
        api_input = text[:_MAX_INPUT_CHARS]
        ...
        try:
                ...
                async with session.post(
                    ...
                    if resp.status == 200:
                        ...
                        if embedding.shape != (_EMBEDDING_DIM,):
                            logger.warning(
                                "[_get_embedding] unexpected embedding dim: %s "
                                "(expected %d)",
                                embedding.shape,
                                _EMBEDDING_DIM,
                            )
                    ...
                    logger.warning(
                        "[_get_embedding] OpenAI API error: HTTP %d %s",
                        resp.status,
                        body[:200],
                    )
                    return None
            ...
            logger.warning(
                "[_get_embedding] OpenAI API timeout (%ds)",
                _OPENAI_TIMEOUT_SECONDS,
            )
            return None
        except aiohttp.ClientError as exc:
            logger.warning("[_get_embedding] HTTP client error: %s", exc)
            return None
        except Exception as exc:
            logger.warning("[_get_embedding] unexpected error: %s", exc)
            return None
