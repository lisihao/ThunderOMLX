...
分析每个 token 生成的耗时，识别瓶颈：
- KV Cache 加载时间
- Attention 计算时间
- Block 索引查找时间
- 总体 TPOT (Time Per Output Token)
"""
import sys
import time
...
def profile_function(name: str):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            PERF_STATS[name].append(elapsed)
            return result
        return wrapper
    return decorator
...
async def run_profile_test():
    """运行性能分析测试"""
    import asyncio
    from omlx.admin.benchmark import run_benchmarks
    ...
    for result in results:
        print(f"\n✅ Benchmark 结果: pp{result.pp_len}/tg{result.max_tokens}")
        print(f"  TTFT: {result.ttft_ms:.1f}ms")
        print(f"  TPOT: {result.tpot_ms:.2f} ms/tok")
        print(f"  Generation TPS: {result.gen_tps:.1f} tok/s")
